...
SingleInstance::StartResult SingleInstance::start() {
    const auto identity =
        QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation).toUtf8();
    const auto name =
        QStringLiteral("better-pip-") +
        QString::fromLatin1(
    ...
    if (existing.error() == QLocalSocket::ServerNotFoundError ||
        ...
        return server_.listen(name) ? StartResult::Primary : StartResult::Failed;
    }
