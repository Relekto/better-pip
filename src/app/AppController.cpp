...
AppController::AppController(bool smokeTest, QObject *parent)
    ...
    trayMenu_.addAction(QStringLiteral("Open controls / unlock"), this,
                        &AppController::showControls);
...
WindowSources *AppController::sources() {
    return &sources_;
}
bool AppController::active() const {
    return capture_.active();
}
bool AppController::hasFrame() const {
    return capture_.hasFrame();
}
bool AppController::locked() const {
    return locked_;
}
...
QString AppController::sourceTitle() const {
    return capture_.title();
}
QString AppController::message() const {
    return message_;
}
QString AppController::shortcut() const {
    return wayland() && shortcut_.registered() ? shortcut_.description()
                                               : preferences_.lockShortcut;
}
QString AppController::shortcutStatus() const {
    if (shortcut_.pending()) {
        return QStringLiteral("Waiting for your desktop's shortcut permission…");
    }
    return shortcut_.registered() ? QStringLiteral("Global shortcut active")
                                  : QStringLiteral("Use Open controls to unlock");
}
int AppController::scaleMode() const {
    return static_cast<int>(preferences_.scaleMode);
}
int AppController::theme() const {
    return static_cast<int>(preferences_.theme);
}
int AppController::opacity() const {
    return preferences_.opacity;
}
bool AppController::preserveAspect() const {
    return preferences_.preserveAspect;
}
int AppController::pipWidth() const {
    return overlay_ ? overlay_->width() : preferences_.geometry.width();
}
int AppController::pipHeight() const {
    return overlay_ ? overlay_->height() : preferences_.geometry.height();
}
void AppController::setScaleMode(int value) {
    if (value < 0 || value > 2 || scaleMode() == value) {
        return;
    }
...
void AppController::setOpacity(int value) {
    ...
    if (overlay_) {
        overlay_->setOpacity(static_cast<qreal>(value) / 100.0);
    }
    save();
    emit changed();
}
...
void AppController::toggleLock() {
    ...
    locked_ = !locked_;
    if (locked_) {
        lockedGeometry_ = overlay_->geometry();
    }
    applyWindowState();
    emit changed();
}
...
void AppController::applyWindowState() {
    ...
    QScopedValueRollback guard(applyingGeometry_, true);
    if (overlay_->flags() != flags) {
        overlay_->setFlags(flags);
    }
    overlay_->setGeometry(geometry);
    overlay_->setVisible(capture_.active());
}
void AppController::geometryUpdated() {
    if (!overlay_ || applyingGeometry_) {
        return;
    }
    QScopedValueRollback guard(applyingGeometry_, true);
    if (locked_) {
    ...
    const auto sourceSize = capture_.frameSize();
    if (preferences_.preserveAspect && !sourceSize.isEmpty()) {
        overlay_->resize(constrainedSize(overlay_->size(), static_cast<double>(sourceSize.width()) /
                                                               sourceSize.height()));
    }
