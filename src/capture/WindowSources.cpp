...
QString WindowSources::filter() const {
    return filter_;
}
int WindowSources::count() const {
...
void WindowSources::refresh() {
    ...
    for (const auto &window : QWindowCapture::capturableWindows()) {
        ...
        const auto found =
            std::find_if(sources_.cbegin(), sources_.cend(),
    ...
    if (next.size() == sources_.size() &&
        std::equal(
            next.cbegin(), next.cend(), sources_.cbegin(), [](const Source &a, const Source &b) {
