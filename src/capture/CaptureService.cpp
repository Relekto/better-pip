...
void CaptureService::setVideoSink(QVideoSink* sink) {
    ...
    if (sink) {
        frameConnection_ = connect(sink, &QVideoSink::videoFrameChanged, this,
...
void CaptureService::start(const QCapturableWindow& window) {
    stop();
    if (!window.isValid()) {
        emit failed(QStringLiteral("This window is no longer available. Refresh and choose again."));
        return;
    }
    source_ = window;
...
void CaptureService::fail(const QString& message) {
    stop();
    emit failed(message.isEmpty() ? QStringLiteral("Capture is unavailable for this window.") : message);
}
bool CaptureService::active() const { return active_; }
bool CaptureService::hasFrame() const { return hasFrame_; }
QSize CaptureService::frameSize() const { return frameSize_; }
QString CaptureService::title() const { return title_; }
}
