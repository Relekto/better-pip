...
QRect visibleGeometry(QRect requested, const QRect& available) {
    ...
    requested.moveLeft(std::clamp(requested.left(), available.left(),
                                  available.right() - requested.width() + 1));
    requested.moveTop(std::clamp(requested.top(), available.top(),
                                 available.bottom() - requested.height() + 1));
    return requested;
}
...
QSize constrainedSize(QSize requested, double aspect) {
    ...
    if (!std::isfinite(aspect) || aspect < 160.0 / 8192.0 || aspect > 8192.0 / 90.0)
        return requested;
    const double width = std::clamp(static_cast<double>(requested.width()),
                                    std::max(160.0, 90.0 * aspect),
