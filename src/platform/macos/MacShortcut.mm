...
public:
    ...
    bool bind(const QKeySequence& sequence, QString& error) override {
        ...
        constexpr std::array<UInt32, 12> functions{
        ...
        if (mods.testFlag(Qt::ControlModifier)) modifiers |= cmdKey;
        if (mods.testFlag(Qt::MetaModifier)) modifiers |= controlKey;
        if (mods.testFlag(Qt::AltModifier)) modifiers |= optionKey;
        if (mods.testFlag(Qt::ShiftModifier)) modifiers |= shiftKey;
...
private:
    static OSStatus handle(EventHandlerCallRef, EventRef event, void* context) {
        ...
        if (GetEventParameter(event, kEventParamDirectObject, typeEventHotKeyID,
                              nullptr, sizeof(identifier), nullptr, &identifier) == noErr &&
