...
class WindowsShortcut final : public ShortcutBackend, public QAbstractNativeEventFilter {
  public:
    ...
    bool bind(const QKeySequence &sequence, QString &error) override {
        ...
        if (nativeKey == 0) {
            error = QStringLiteral("Choose a letter, number, function key, Space, Home, End, "
                                   "Insert, Delete, or Pause.");
            return false;
        }
        ...
        if (!RegisterHotKey(nullptr, next, modifiers, nativeKey)) {
            error =
                QStringLiteral("That shortcut is reserved or already used by another application.");
            return false;
        }
        if (id_ != 0) {
            UnregisterHotKey(nullptr, id_);
