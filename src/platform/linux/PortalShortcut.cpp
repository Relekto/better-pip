...
PortalShortcut::PortalShortcut(GlobalShortcut& owner) : owner_(owner) {
    ...
    QDBusConnection::sessionBus().connect(service, path, interface, QStringLiteral("Activated"),
...
bool PortalShortcut::asynchronous() const { return true; }
bool PortalShortcut::bind(const QKeySequence& sequence, QString& error) {
    ...
    if (modifiers.testFlag(Qt::ControlModifier)) parts << QStringLiteral("CTRL");
    if (modifiers.testFlag(Qt::AltModifier)) parts << QStringLiteral("ALT");
    if (modifiers.testFlag(Qt::ShiftModifier)) parts << QStringLiteral("SHIFT");
    if (modifiers.testFlag(Qt::MetaModifier)) parts << QStringLiteral("LOGO");
    ...
    connect(request_, &PortalRequest::finished, this,
        [this, trigger](uint code, const QVariantMap& results, const QString& message) {
            request_.clear();
            if (code != 0) { reject(message); return; }
            const auto value = results.value(QStringLiteral("session_handle"));
            candidate_ = value.canConvert<QDBusObjectPath>()
                       ? value.value<QDBusObjectPath>().path() : value.toString();
            if (candidate_.isEmpty()) { reject(QStringLiteral("The desktop did not create a shortcut session.")); return; }
            bindCandidate(trigger);
        });
    return true;
}
void PortalShortcut::bindCandidate(const QString& trigger) {
    const PortalBindings bindings{{QStringLiteral("toggle-lock"),
    ...
    request_ = PortalRequest::call(interface, QStringLiteral("BindShortcuts"),
        {QVariant::fromValue(QDBusObjectPath(candidate_)), QVariant::fromValue(bindings), QString{}}, {}, this);
    connect(request_, &PortalRequest::finished, this,
        [this](uint code, const QVariantMap& results, const QString& message) {
            request_.clear();
            if (code != 0) { reject(message); return; }
            const auto bindings = qdbus_cast<PortalBindings>(results.value(QStringLiteral("shortcuts")));
            QString description;
            bool found = false;
            for (const auto& binding : bindings) {
                if (binding.id == QStringLiteral("toggle-lock")) {
                    found = true;
                    description = binding.options.value(QStringLiteral("trigger_description")).toString();
                }
            }
            if (!found) { reject(QStringLiteral("The desktop did not bind the shortcut.")); return; }
            closeSession(session_);
            session_ = std::exchange(candidate_, {});
            QDBusConnection::sessionBus().connect(service, session_,
...
void PortalShortcut::activated(const QDBusObjectPath& session, const QString& id,
...
void PortalShortcut::sessionClosed() {
    session_.clear();
    owner_.invalidate(QStringLiteral("The desktop ended the shortcut session. Use the control window to unlock."));
}
void PortalShortcut::reject(const QString& message) {
    cancelCandidate();
    owner_.completeRegistration(false, message.isEmpty()
        ? QStringLiteral("Global shortcuts are unavailable. Use the control window to unlock.") : message);
}
...
void PortalShortcut::closeSession(const QString& session) {
    ...
    QDBusConnection::sessionBus().disconnect(service, session,
        QStringLiteral("org.freedesktop.portal.Session"), QStringLiteral("Closed"),
        this, SLOT(sessionClosed()));
    const auto message = QDBusMessage::createMethodCall(service, session,
