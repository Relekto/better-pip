...
PortalRequest *PortalRequest::call(const QString &interface, const QString &method,
    ...
    request->path_ =
        QStringLiteral("/org/freedesktop/portal/desktop/request/") + sender + '/' + token;
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, request,
            [request](QDBusPendingCallWatcher *completed) {
                ...
                if (reply.isError()) {
                    ...
                    request->complete(
                        2, {},
                        QStringLiteral("The desktop returned an unexpected request handle."));
                }
            });
...
void PortalRequest::cancel() {
    ...
    if (!path_.isEmpty()) {
        const auto message = QDBusMessage::createMethodCall(
            QStringLiteral("org.freedesktop.portal.Desktop"), path_,
