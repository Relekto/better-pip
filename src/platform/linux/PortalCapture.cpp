...
QString objectPath(const QVariant &value) {
    return value.canConvert<QDBusObjectPath>() ? value.value<QDBusObjectPath>().path()
                                               : value.toString();
}
}
struct PortalCapture::Private {
    ...
    void select() {
        ...
        request =
            PortalRequest::call(screenCast, QStringLiteral("SelectSources"),
                         ...
                         [this](uint code, const QVariantMap &, const QString &message) {
                             ...
                             if (code != 0) {
                                 error(message.isEmpty()
                                           ? QStringLiteral("Window sharing is unavailable.")
                                           : message);
                                 return;
                             }
                             begin();
    ...
    void begin() {
        request = PortalRequest::call(screenCast, QStringLiteral("Start"),
                                      {QVariant::fromValue(QDBusObjectPath(session)), QString{}},
                                      {}, &owner);
        QObject::connect(
            request, &PortalRequest::finished, &owner,
            [this](uint code, const QVariantMap &results, const QString &message) {
                request.clear();
                if (code != 0) {
                    error(message.isEmpty() ? QStringLiteral("Window sharing could not start.")
                                            : message);
                    return;
                }
                const auto argument =
                    results.value(QStringLiteral("streams")).value<QDBusArgument>();
                uint node = PW_ID_ANY;
                ...
                argument.endArray();
                if (node == PW_ID_ANY) {
                    error(QStringLiteral("No window stream was returned."));
                    return;
                }
    ...
    void openRemote(uint node, const QString &serial) {
        ...
        auto *watcher =
            new QDBusPendingCallWatcher(QDBusConnection::sessionBus().asyncCall(message), &owner);
        QObject::connect(watcher, &QDBusPendingCallWatcher::finished, &owner,
                         [this, node, serial, epoch](QDBusPendingCallWatcher *completed) {
                             ...
                             if (fd < 0) {
                                 error(QStringLiteral("Could not open the shared window stream."));
                                 return;
                             }
    ...
    static void process(void *data) {
        ...
        case SPA_VIDEO_FORMAT_BGRx:
            pixelFormat = QVideoFrameFormat::Format_BGRX8888;
            break;
        case SPA_VIDEO_FORMAT_RGBA:
            pixelFormat = QVideoFrameFormat::Format_RGBA8888;
            break;
        case SPA_VIDEO_FORMAT_RGBx:
            pixelFormat = QVideoFrameFormat::Format_RGBX8888;
            break;
        default:
            return;
        }
        QVideoFrame frame(QVideoFrameFormat(
            QSize(static_cast<int>(width), static_cast<int>(height)), pixelFormat));
        ...
        QMetaObject::invokeMethod(
            ...
            [&self, epoch] {
                ...
                if (frame.isValid()) {
                    emit self.owner.frameReady(frame);
                }
            },
            Qt::QueuedConnection);
    }
    void connectStream(int fd, uint node, const QString &serial) {
        ...
        if (!loop) {
            close(fd);
            error(QStringLiteral("Could not create the capture loop."));
            return;
        }
        context = pw_context_new(pw_thread_loop_get_loop(loop), nullptr, 0);
        if (!context) {
            close(fd);
            error(QStringLiteral("Could not create the capture context."));
            return;
        }
        core = pw_context_connect_fd(context, fd, nullptr, 0);
        if (!core) {
            error(QStringLiteral("Could not connect to PipeWire."));
            return;
        }
        auto *properties = pw_properties_new(PW_KEY_MEDIA_TYPE, "Video", PW_KEY_MEDIA_CATEGORY,
                                             "Capture", PW_KEY_MEDIA_ROLE, "Screen", nullptr);
        if (!properties) {
            error(QStringLiteral("Could not allocate stream properties."));
            return;
        }
        if (!serial.isEmpty()) {
            pw_properties_set(properties, PW_KEY_TARGET_OBJECT, serial.toUtf8().constData());
        }
        stream = pw_stream_new(core, "Better PiP", properties);
        if (!stream) {
            error(QStringLiteral("Could not create the window stream."));
            return;
        }
        ...
        spa_pod_builder builder =
            SPA_POD_BUILDER_INIT(storage.data(), static_cast<uint32_t>(storage.size()));
        const spa_pod *parameter = static_cast<spa_pod *>(spa_pod_builder_add_object(
...
PortalCapture::PortalCapture(QObject *parent)
    : QObject(parent), d_(std::make_unique<Private>(*this)) {}
...
void PortalCapture::start() {
    ...
    d_->request =
        PortalRequest::call(screenCast, QStringLiteral("CreateSession"), {}, options, this);
    connect(d_->request, &PortalRequest::finished, this,
            [this](uint code, const QVariantMap &results, const QString &message) {
                d_->request.clear();
                if (code != 0) {
                    d_->error(message.isEmpty() ? QStringLiteral("Window sharing is unavailable.")
                                                : message);
                    return;
                }
                d_->session = objectPath(results.value(QStringLiteral("session_handle")));
                if (d_->session.isEmpty()) {
                    d_->error(QStringLiteral("The desktop did not create a sharing session."));
                    return;
                }
                QDBusConnection::sessionBus().connect(
                    service, d_->session, QStringLiteral("org.freedesktop.portal.Session"),
...
void PortalCapture::sessionClosed() {
    d_->error(QStringLiteral("Window sharing ended in your desktop."));
}
}
