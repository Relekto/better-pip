...
class LinuxShortcut final : public ShortcutBackend {
  public:
    ...
    bool bind(const QKeySequence &sequence, QString &error) override {
        if (!display_) {
            error = QStringLiteral("Global shortcuts are unavailable in this session. Use the "
                                   "control window to unlock.");
            return false;
        }
        const auto combination = sequence[0];
        const auto name =
            QKeySequence(combination.key()).toString(QKeySequence::PortableText).toLatin1();
        const auto symbol =
            combination.key() == Qt::Key_Space ? XK_space : XStringToKeysym(name.constData());
        ...
        for (auto extra : ignoredModifiers_) {
            XGrabKey(display_, code, modifiers | extra, DefaultRootWindow(display_), False,
